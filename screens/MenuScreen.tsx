...
export default function MenuScreen({ userId, userEmail, isProfileComplete }: MenuScreenProps) {
  ...
  useEffect(() => {
      ...
      if (triggered) {
          ...
          Alert.alert("Scheduled Order Ready! ⏰", `Token(s) ${newReadyOrders.join(', ')} are 15 mins away and have automatically moved to the Live Queue!`);
      }
        ...
        .on('postgres_changes', { event: 'INSERT', schema: 'public', table: 'errands' }, (payload) => {
             ...
             if (newOrder.shop_name === adminShopName) {
                 ...
                 if (!newOrder.requested_time || newOrder.requested_time === 'ASAP' || isTimeReady(newOrder.requested_time, new Date())) {
                     ...
                     Alert.alert("🚨 NEW ORDER", `Token: ${newOrder.token_no}`);
                 }
             }
        })
      ...
      const channel = supabase.channel('student_listener')
        .on('postgres_changes', { event: 'UPDATE', schema: 'public', table: 'errands', filter: `student_id=eq.${userId}` }, (payload) => {
             ...
             if (newOrder.status === 'cooking') {
                 ...
                 Alert.alert("Order Accepted ✅", `The vendor has started preparing your food!`); 
             }
             else if (newOrder.status === 'ready') {
                 Speech.speak("Order is ready for pickup.", { rate: 1.0 });
                 Alert.alert("Order Ready 🍔", "Your food is ready. Please pick it up!");
             }
             else if (newOrder.status === 'cancelled') {
                 Speech.speak("Order declined.", { rate: 1.0 });
                 Alert.alert("Order Cancelled ❌", "The vendor declined your order.");
             }
      }).subscribe();
      return () => { supabase.removeChannel(channel); };
  }, [userId, isAdmin]);
  ...
  async function resolveCancelledOrder(id: string) {
      Alert.alert("Issue Resolved?", "Have you visited the store and resolved the payment/order issue? This will remove the order from the list.", [
          ...
          { text: "Yes, Resolved", onPress: async () => {
              ...
              if (!error) {
                  Alert.alert("Resolved ✅", "Order has been cleared from the history.");
                  fetchMyOrders(); 
              } else {
  ...
  async function updateItemDetails() {
      ...
      if (!error) { Alert.alert("Success", "Item updated!"); setEditingItem(null); setNewPrice(''); setNewName(''); fetchShopData(); } 
  }

  async function addNewMenuItem() {
      if (!newItem.name || !newItem.price) { Alert.alert("Missing Info"); return; }
      ...
      if (!error) { Alert.alert("Success", "Item Added!"); setShowAddMenuModal(false); setNewItem({ name: '', price: '', category: 'Snacks', is_veg: true }); fetchShopData(); } else { Alert.alert("Error", error.message); }
  }
  ...
  const updateCart = (id: string, delta: number) => {
    if (!isAdmin && delta > 0) {
        if (!isProfileComplete) {
            Alert.alert("Profile Incomplete 🛑", "You must complete your profile in the Profile tab before you can order food.");
            return;
        }
        if (!shopStatus) {
            Alert.alert("Shop Closed 🛑", "The vendor is temporarily not accepting orders. Please try again later.");
            return;
        }
    }
  ...
  async function placeOrder(method: 'cash' | 'online') {
    if (!isAdmin) {
        ...
        if (!shopStatus) {
            Alert.alert("Shop Closed 🛑", "The vendor is temporarily not accepting orders.");
            return;
        }
    }

    if (orderType === 'delivery' && !deliveryAddress.trim()) { Alert.alert("Address Missing", "Please enter your Hostel Block & Room No."); return; }
  ...
  const openVendorPayment = (upiId: string, name: string, amount: number, mode: 'RUNNER' | 'SERVER', runnerId?: string) => {
    if (!upiId) { Alert.alert("No UPI Found", `The user hasn't added their UPI ID yet.`); return; }
    const randomPaisa = Math.floor(Math.random() * 90) + 10;
  ...
  const handlePaymentSuccess = async () => {
    if (!userUTR || userUTR.length < 4) { Alert.alert("Invalid UTR", "Please enter the last 4 digits of the payment ref."); return; }
    ...
    if (paymentMode === 'STUDENT') {
        await saveOrder(pendingTotal, pendingToken, 'online', userUTR);
         ...
         const { error } = await supabase.from('errands')
            ...
            .eq('status','delivered')
            .eq('is_payout_complete', false);
         if (!error) { Alert.alert("Proof Sent", `UTR ${userUTR} submitted. Runner paid.`); fetchPayouts(); }
    } 
    else if (paymentMode === 'SERVER') {
         Alert.alert("Recorded", `Server Fees UTR ${userUTR} saved.`); 
    }
  ...
  async function saveOrder(total: number, token: number, method: string, utr: string) {
      ...
      if (error) { Alert.alert("Error", "Failed."); return; }
      ...
      Alert.alert("Order Sent! ⏳", `Token: #${token}\n\nWaiting for vendor acceptance...`);
      fetchMyOrders();
  }
