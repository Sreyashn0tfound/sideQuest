...
import { 
  ...
  Modal, TextInput, KeyboardAvoidingView, Platform, Image, ActivityIndicator, FlatList, Dimensions, Linking 
} from 'react-native';
...
export default function ProfileScreen({ session, isLockdown }: { session: any, isLockdown?: boolean }) {
  ...
  async function markStoryTaskDone() {
      ...
      if (!error) {
          Alert.alert("Awesome! 🚀", "Admin will verify your DM and add Mana soon.");
          fetchProfile();
      }
  }
  ...
  async function toggleDevMode(val: boolean) {
    ...
    Alert.alert(val ? "Dev Mode ON 👨‍💻" : "Dev Mode OFF 🚫", val ? "Payments bypassed." : "Real payments active.");
  }
  ...
  async function verifyStudent(studentId: string, approve: boolean) {
      if (approve) {
          ...
          Alert.alert("Banned", "ID Rejected. User BANNED. 🚫");
      }
  ...
  async function unbanStudent(studentId: string) {
      ...
      Alert.alert("Unbanned", "User unbanned. They must upload ID again to order.");
      fetchBannedUsers(); fetchVerifications();
  }

  async function pickAndUploadID() {
      if (profile?.is_banned) { Alert.alert("Account Banned", "You cannot upload an ID while banned. Appeal via Chat."); return; }
      if (profile?.is_verified) { Alert.alert("Verified", "Your ID is already verified. You cannot change it."); return; }
      ...
      if (!image.base64) { Alert.alert("Error", "Could not process image data."); return; }
      
      if ((image.base64.length * 0.75) / (1024 * 1024) > 2) { 
          Alert.alert("File Too Large", "Max 2MB allowed."); 
          return; 
      }
      ...
      try {
          ...
          if (looksLikeId && nameMatch) {
               ...
               if (ocrAttempts < 1) {
                   ...
                   Alert.alert("Image Unclear 📸", "We couldn't clearly detect a Student ID.\n\nPlease ensure good lighting, no glare, and try again.");
                   return; // STOP HERE
               } else {
                   // Second Failure: Allow upload but force manual review
                   Alert.alert("Auto-Check Failed", "We still couldn't read your ID. Sending for Manual Admin Verification.");
                   isAiVerified = false;
               }
          } else {
               // Looks like ID but name didn't match
               Alert.alert("Name Mismatch", "ID text detected, but name didn't match profile. Sending for Manual Verification.");
               isAiVerified = false;
          }
          ...
          if (isAiVerified) {
              Alert.alert("Verified! 🤖", `ID verified automatically. Welcome, ${profile.full_name}!`);
          } else {
              // Only show if we didn't show the retry alert earlier
              if (looksLikeId) Alert.alert("Uploaded 📤", "ID Uploaded for Admin Verification.");
          }
  ...
  const sendEmailOtp = async () => {
      ...
      if (error) {
          ...
          setEmailSent(true);
          Alert.alert("OTP Sent 📧", `Check your email (${session.user.email}) for the 6-digit code.`);
      }
  };

  const verifyEmailOtp = async () => {
      if (!emailCode || emailCode.length < 6) {
          Alert.alert("Invalid Code", "Please enter the 6-digit OTP from your email.");
          return;
      }
      ...
      if (error) {
          ...
          setVerifyingEmail(false);
          Alert.alert("Success! ✅", "Account securely verified and locked.");
          fetchProfile();
      }
  };

  async function saveProfile() {
      if (!tempName.trim()) { Alert.alert("Error", "Name is required."); return; }
      if (!tempPhone.trim() || tempPhone.length < 10) { Alert.alert("Error", "A valid Phone Number is required."); return; }
      if (!tempRecoveryEmail.trim() || !tempRecoveryEmail.includes('@')) { Alert.alert("Error", "A valid Recovery Email is required."); return; }
  ...
  async function savePayment() {
      if (!tempUpi.trim()) { Alert.alert("Error", "Enter valid UPI ID."); return; }
      const { error } = await supabase.from('profiles').update({ upi_id: tempUpi }).eq('id', session.user.id);
      if (error) Alert.alert("Error", error.message);
      else { Alert.alert("Success", "Payment Method Saved! 💳"); setShowPayment(false); fetchProfile(); }
  }
  ...
  async function handleWithdraw() {
      if ((profile?.mana_balance || 0) < 1000) {
          Alert.alert("Low Balance", "You need at least 1000 Mana to withdraw.");
          return;
      }
      ...
      if (error) {
          Alert.alert("Error", "Could not deduct Mana.");
          return;
      }
      ...
      Linking.openURL(url).catch(err => {
          Alert.alert("Error", "Could not open mail app. Please manually email office.sidequest26@gmail.com");
      });

      Alert.alert("Success", "1000 Mana deducted. Email draft created.");
      fetchProfile();
  }
  ...
  return (
    <View style={[styles.container, { backgroundColor: colors.background }]}>
      <View style={styles.header}>
        ...
        <TouchableOpacity 
            onPress={() => {
                Alert.alert("Sign Out", "Are you sure?", [{ text: "Cancel", style: "cancel" }, { text: "Sign Out", style: "destructive", onPress: handleSignOut }]);
            }} 
