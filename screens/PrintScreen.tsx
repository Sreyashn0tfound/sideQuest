...
export default function PrintScreen({ userId, userEmail, isProfileComplete }: PrintScreenProps) {
  ...
  function initiateOrder() {
    if (files.length === 0) { Alert.alert("No File", "Please select a PDF."); return; }
    if (files.some(f => !f.pageCount || parseInt(f.pageCount) <= 0)) { Alert.alert("Missing Pages", "Please enter page count manually for all files."); return; }
    ...
    files.forEach(f => {
        ...
        if (f.color === 'mixed') {
            ...
            if (cPgs + bPgs === 0) {
                Alert.alert("Missing Info", `Please enter the page numbers for the mixed file: ${f.name}`);
  ...
  async function finalizeOrder() {
    if (paymentUtr.length < 4) { Alert.alert("Invalid UTR", "Please enter last 4 digits."); return; }
    setSubmitting(true);
  ...
  return (
    <KeyboardAvoidingView behavior={Platform.OS === 'ios' ? 'padding' : undefined} style={[styles.container, { backgroundColor: colors.background }]}>
      <View style={{ paddingTop: 60, paddingHorizontal: 20, paddingBottom: 10, flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center' }}>
        ...
        <View style={{flexDirection:'row', backgroundColor:'#333', borderRadius: 15, padding: 2}}>
            {!isAdmin ? (
                <>
                    <TouchableOpacity 
                        onPress={() => {
                            if (!isShopOpen) Alert.alert("Closed", "The shop is not accepting orders right now.");
