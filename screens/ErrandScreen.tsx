...
import { 
  ...
  RefreshControl, Dimensions, Linking, TextInput 
} from 'react-native';
...
export default function ErrandScreen({ userId, isProfileComplete }: ErrandScreenProps) {
  ...
  function confirmAccept(item: any) {
      if (!isProfileComplete) {
          Alert.alert("Profile Incomplete 🛑", "Runners must be verified. Go to Profile tab.");
          return;
      }
      ...
      Alert.alert(
          ...
          [
              ...
              { text: "I Accept 🚀", onPress: () => acceptErrand(item) } 
          ]
      );
  }
  ...
  async function acceptErrand(item: any) {
      ...
      if (error) {
          Alert.alert("Error", "Delivery already taken by another runner!");
      } else { 
          ...
          Alert.alert("Accepted! 🎮", "Pick up the order and deliver it to get paid."); 
          fetchErrands(); 
          setActiveTab('mine'); 
      }
  }
  ...
  function callStudent(phone: string) {
      if (!phone) {
          Alert.alert("Error", "Phone number hidden.");
          return;
      }
  ...
  async function verifyAndComplete(item: any) {
      ...
      if (!enteredOtp || enteredOtp.length !== 4) {
          Alert.alert("Invalid OTP", "Enter the 4-digit code from the student.");
          return;
      }

      if (enteredOtp !== item.delivery_otp) {
          Alert.alert("Wrong Code ❌", "Ask the student for the correct 4-digit OTP.");
          return;
      }
      ...
      if (!error) {
        ...
        Alert.alert("Delivery Complete! 💰", `You earned ₹20. Check the Payouts tab in the vendor's dashboard.`);
        fetchErrands();
      } else {
  ...
  return (
    <View style={[styles.container, { backgroundColor: colors.background }]}>
      <View style={styles.header}>
        ...
        <TouchableOpacity 
            onPress={() => Alert.alert("Coming Soon 🚀", "Custom Errands and peer-to-peer tasks are dropping in the next update!")} 
